...
const EventsCard: React.FC<EventsCardProps> = ({ Item, onClick }) => {
  return (
    <article onClick={onClick} className="card-shine-2 group cursor-pointer">
      ...
      <Copy delay={0.2}>
        <p className="text-secondary mt-4 text-xs font-semibold duration-200 ease-in-out lg:mt-6 lg:text-sm xl:mt-8 px-3">
          {Item.role}
        </p>
      </Copy>
      ...
      <Copy delay={0.4}>
        <h3 className="text-flintstone text-sm duration-200 ease-in-out lg:text-base px-3">
          {Item.description}
        </h3>
      </Copy>
      <Copy delay={0.5}>
