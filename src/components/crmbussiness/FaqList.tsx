...
const FaqList: React.FC<FaqListProps> = ({
  ...
  return (
    ...
    >
      ...
      <button
        className={`flex w-full cursor-pointer items-start justify-between gap-5 px-2 py-3 text-start text-base leading-[127%] font-black ${currentVariant.question} ${classNameQue} ${isOpen ? "faq-opened" : ""} `}
      >
        {data.question}
      ...
      <AnimateHeight duration={500} height={isOpen ? "auto" : 0}>
        <p
          className={`max-w-[1113px] px-2 pt-1 pb-3 text-sm leading-[126%] ${currentVariant.answer} ${classNameAnswer} ${isOpen ? "text-visible" : ""} `}
        >
          {data.answer
            ?.split("<br/>")
