...
const getBaseRateForIndustry = (industryName?: string): number => {
  ...
  if (industryName) {
    ...
    if (normalizedName.includes('electrical')) {
      ...
      baseRate = 34.25;
    } else if (normalizedName.includes('steel') || normalizedName.includes('metal')) {
      baseRate = 41.5;
...
const getGrowthTrend = (yearOffset: number, industryName?: string): number => {
  ...
  if (industryName) {
    ...
    if (normalizedName.includes('electrical') || normalizedName.includes('technology')) {
      annualGrowth = 2.3;
    } else if (normalizedName.includes('hvac') || normalizedName.includes('mechanical')) {
      annualGrowth = 2.1;
    } else if (normalizedName.includes('plumbing')) {
      annualGrowth = 1.9;
    } else if (normalizedName.includes('concrete') || normalizedName.includes('masonry')) {
      annualGrowth = 1.6;
    }
  }
